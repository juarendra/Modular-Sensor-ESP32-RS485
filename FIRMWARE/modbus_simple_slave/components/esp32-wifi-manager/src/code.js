...
var label = document.getElementById('modbusStatus');
...
docReady(async function () {
  ...
  gel("storeSetting").addEventListener(
    "click", 
    (e) => {
      performConnect("", "MODBUS");
    },
     false
  );
  gel("storeSettingMQTT").addEventListener(
    "click", 
    (e) => {
      performConnect("", "MQTT");
    },
...
async function performConnect(conntype, protocol) {
  ...
  if (protocol == "WIFI"){
    ...
    stopCheckStatusInterval();
    stopRefreshAPInterval();
    slaveAddr = gel("slaveAddr").value;
    sensorType = gel("sensorTypeSelect").value;
    baudrate = gel("baudrate").value;

    var response = await fetch("modbus.json", {
      method: "POST",
      headers: {
        "Content-Type": "application/json/modbus",
        "customSlaveAddr": slaveAddr,
        "customSensorType": sensorType,
        "customBaudrate": baudrate,
      },
      body: { timestamp: Date.now() },
    });
    if (response.ok){
      document.getElementById("modbusStatus").innerHTML = "Succes save Modbus Parameter";
      modal.style.display = "block";
    }
    else{
      document.getElementById("modbusStatus").innerHTML = "Something error in here";
      modal.style.display = "block";
    }
    //now we can re-set the intervals regardless of result
    startCheckStatusInterval();
    ...
    stopCheckStatusInterval();
    stopRefreshAPInterval();
    mqttpublish = gel("mqttpublish").value;
    mqttBroker = gel("mqttBroker").value;
    mqttPort = gel("mqttPort").value;
    mqttUsername = gel("mqttUsername").value;
    mqttPassword = gel("mqttPassword").value;
    mqttTopic = gel("mqttTopic").value;

    var response = await fetch("mqtt.json", {
      method: "POST",
      headers: {
        "Content-Type": "application/json/mqtt",
        "customMqttpublish": mqttpublish,
        "customMqttBroker": mqttBroker,
        "customMqttPort": mqttPort,
        "customMqttUsername": mqttUsername,
        "customMqttPassword": mqttPassword,
        "customMqttTopic": mqttTopic,
      },
      body: { timestamp: Date.now() },
    });
    if (response.ok){
      document.getElementById("modbusStatus").innerHTML = "Succes save MQTT Parameter";
      modal.style.display = "block";
    }
    else{
      document.getElementById("modbusStatus").innerHTML = "Something error in here";
